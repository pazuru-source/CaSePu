...
def calculate_chance_of_profit(stock_price, strike, implied_vol, days_to_expiry, risk_free_rate=0.04):
    ...
    try:
        ...
        d2 = (math.log(stock_price / strike) + (risk_free_rate - 0.5 * vol**2) * T) / (vol * math.sqrt(T))
...
def find_cash_secured_puts(ticker, target_date_str, current_price):
    ...
    suitable_options = []
    try:
        # Check if requested date exists, otherwise use the first available one
        available_options = ticker.options
        if not available_options:
            return []
            
        final_target = target_date_str if target_date_str in available_options else available_options[0]
        
        options_chain = ticker.option_chain(final_target)
        ...
        for index, put in puts.iterrows():
            ...
            # Ensure 'bid' is not NaN and is positive
            ...
            if bid >= required_premium:
                ...
                try:
                    exp_dt = datetime.strptime(final_target, '%Y-%m-%d')
                    today = datetime.now()
                    days_to_expiry = (exp_dt - today).days
                except Exception:
                    days_to_expiry = 7 # fallback to 1 week
                
                chance_of_profit = calculate_chance_of_profit(current_price, strike, implied_vol, days_to_expiry)
...
def process_single_ticker(ticker_symbol, target_exp, extra_tickers, fav_tickers):
    ...
    try:
        ...
        if not ticker_obj:
            logger.warning(f"Could not initialize ticker object for {ticker_symbol}")
            return None
        ...
        if price_ok and (meets_criteria or is_manual):
            puts = find_cash_secured_puts(ticker_obj, target_exp, fundamentals['current_price'])
            if puts:
                return {
...
def analyze():
    ...
    logger.info(f"Targets: {targets} | Total tickers to analyze: {len(all_tickers)}")

    if not all_tickers:
        return jsonify({'status': 'error', 'message': 'No tickers selected for analysis.'}), 400

    expiration_date_ts = get_next_friday_expiration()
    target_exp = datetime.fromtimestamp(expiration_date_ts).strftime('%Y-%m-%d')
    ...
    for ticker_symbol in all_tickers:
        ...
        if price_ok and (meets_criteria or is_manual):
            ...
            puts = find_cash_secured_puts(ticker_obj, target_exp, fundamentals['current_price'])
            if puts:
                stock_data = {'ticker': ticker_symbol, 'fundamentals': fundamentals or {}, 'puts': puts}
    ...
    logger.info(f"Analysis finished. Found {len(suitable_stocks_for_csp)} stocks. Duration: {analysis_end_time - analysis_start_time}")
...
def scan_start():
    ...
    logger.info(f"Scan Session Start | Targets: {targets} | Total tickers to scan: {len(all_tickers)}")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.error(f"Error processing {symbol} in parallel scan: {e}")
